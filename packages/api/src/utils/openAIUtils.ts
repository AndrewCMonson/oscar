import { ProjectType, ResponseStyle, TaskPriority, TaskStatus, Tone } from "@prisma/client";
...
export const createProjectParameters = z.object({
  ...
  type: z.enum([ProjectType.CLIENT, ProjectType.INTERNAL, ProjectType.PERSONAL]).describe("The type of project"),
  userId: z.string().describe("The user's id")
})

export const userPreferenceParams = z.object({
  id: z.string().describe("The id of the user preferences model"),
  tone: z.enum([Tone.CONCISE, Tone.FRIENDLY, Tone.PROFESSIONAL]).describe("The user's preferred response tone"),
  responseStyle: z.enum([ResponseStyle.CONVERSATIONAL, ResponseStyle.DIRECT]).describe("The user's preferred response style"),
  preferredLanguage: z.string().describe("The user's preferred default language"),
...
export const createTaskParams = z.object({
  ...
  status: z.enum([TaskStatus.BLOCKED, TaskStatus.COMPLETED, TaskStatus.IN_PROGRESS, TaskStatus.TODO]).describe("The current status of the task"),
  priority: z.enum([TaskPriority.HIGH, TaskPriority.LOW, TaskPriority.MEDIUM, TaskPriority.URGENT]).describe("The priority of the task"),
...
export const openAITools = [
  ...
  zodFunction({name: "updateUserPreferences", parameters: userPreferenceParams}),
  zodFunction({name: "createTask", parameters: createTaskParams})
]

export const handleToolCallFunction = async (toolCallFunctionName: string, toolCallFunctionArgs: object) => {
    ...
    case "updateUserPreferences":
      {
        const updatedPreferences = prismadb.userPreferences.update({
          where: {
            userId: toolCallFunctionArgs.userId as string
          },
          data: {
